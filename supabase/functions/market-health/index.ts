...
function calculateEMA(prices, period) {
  ...
  const k = 2 / (period + 1);
  let ema = prices.slice(0, period).reduce((a, b) => a + b, 0) / period; // SMA for first value
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Calculate Market Trend (^CRSLDX - Nifty 500 Index)
    const trendRes = await fetch('https://query1.finance.yahoo.com/v8/finance/chart/%5ECRSLDX?range=6mo&interval=1d', { headers: { 'User-Agent': 'Mozilla/5.0' } });
    const trendData = await trendRes.json();
    const trendPrices = trendData.chart?.result?.[0]?.indicators?.quote?.[0]?.close?.filter(p => p !== null) || [];
    
    let trend = 'Unknown';
    if (trendPrices.length > 50) {
      const currentPrice = trendPrices[trendPrices.length - 1];
      const ema20 = calculateEMA(trendPrices, 20);
      const ema50 = calculateEMA(trendPrices, 50);
    ...
    for (let i = 0; i < NIFTY_500.length; i += chunkSize) {
      ...
      for (const r of results) {
        const prices = r.response?.[0]?.indicators?.quote?.[0]?.close?.filter(p => p !== null) || [];
        if (prices.length >= 20) {
          const currentPrice = prices[prices.length - 1];
          const ema20 = calculateEMA(prices, 20);
          if (currentPrice > ema20) above20EMA++;
          else below20EMA++;
        }
      }
    }

    const breadthValue = below20EMA === 0 ? 999 : parseFloat((above20EMA / below20EMA).toFixed(2));
    ...
    const marketHealth = { trend, breadthValue, breadthClassification, guidance, lastUpdated: new Date().toISOString().split('T')[0] };

    // 3. Save to all users' settings in Supabase
    const { data: usersSettings } = await supabase.from('settings').select('user_id, data');
    if (usersSettings) {
      for (const row of usersSettings) {
        const newData = { ...row.data, marketHealth };
        await supabase.from('settings').update({ data: newData }).eq('user_id', row.user_id);
      }
    }

    return new Response(JSON.stringify(marketHealth), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
